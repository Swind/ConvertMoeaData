...
XLSX_DATA_DIR = os.path.join(ROOT_DIR, "moea_data")
...
class MoeaSheet:
    ...
    def convert_to_factory_data(self):
...
class MoeaXlsxFile:
    ...
    def convert_all_sheets_factory_data(self) -> List[FactoryData]:
        moea_sheets = self.get_target_sheets()

        sheet_factory_data = {}
        for sheet in moea_sheets:
            sheet_factory_data[sheet.name] = sheet.convert_to_factory_data()

        return sheet_factory_data
...
class AddSectCodeToMoeaData:
    ...
    def convert_moea_xlsx_to_json(self, file_path):
        ...
        image_folder = os.path.join(JSON_DATA_DIR, "images")

        sheets_factory_data = moea_xlsx_file.convert_all_sheets_factory_data()

        dict_data = {}
        for sheet_name, factory_data_list in sheets_factory_data.items():
            dict_data[sheet_name] = []
            for factory_data in factory_data_list:
                # Save factory image to image folder
                if factory_data.image:
                    image_file_name = f"{xlsx_file_name}_{sheet_name}_{factory_data.address}_{factory_data.no}.png"
                    image_file_path = os.path.join(image_folder, image_file_name)
                    with open(image_file_path, "wb") as f:
                        f.write(factory_data.image)

                dict_data[sheet_name].append(factory_data.to_dict())
    ...
    def start(self):
        # List all moea xlsx files in the download from moea
        file_path_list = [
