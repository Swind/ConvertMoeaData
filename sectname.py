...
def get_county_list():
    if os.path.exists(COUNTY_FILE_PATH):
        ...
        raise FileNotFoundError(f"{COUNTY_FILE_PATH} is not existing, please download the metadata first")
...
def get_town_list():
    if os.path.exists(TOWN_FILE_PATH):
        ...
        raise FileNotFoundError(f"{TOWN_FILE_PATH} is not existing, please download the metadata first")

def get_city_town_data():
    cities = get_county_list()
    towns = get_town_list()
